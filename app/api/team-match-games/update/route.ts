...
export async function POST(request: Request) {
  try {
    ...
    const boardNo = Number(formData.get("boardNo") ?? "0");
    const matchType = String(formData.get("matchType") ?? "") as "W" | "S" | "T";
    const winnerSide = String(formData.get("winnerSide") ?? "");
    const team1Label = String(formData.get("team1Label") ?? "");
    const team2Label = String(formData.get("team2Label") ?? "");
    const leftGames = parseIntOrNull(formData.get("leftGames"));
    const rightGames = parseIntOrNull(formData.get("rightGames"));
    ...
    if (format === "T_LEAGUE" && boardNo === 5) {
      const { data: existingGamesBefore5 } = await supabase
      ...
      for (const game of existingGamesBefore5 ?? []) {
    ...
    return NextResponse.redirect(
      new URL(
        `/admin/tournaments/${tournamentId}/divisions/${divisionId}/matches/${matchId}/team-score`,
        request.url
      )
    );
  } catch (error: unknown) {
    const message =
      error instanceof Error ? error.message : "団体戦結果入力に失敗しました。";
